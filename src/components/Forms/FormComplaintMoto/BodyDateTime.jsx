...
function BodyDateTime({
  ...
  useEffect(() => {
    const loadClient = async () => {
      const response = await fetch("http://149.50.132.147:3000/api/clientes/dni", {
        method: "POST",
        body: JSON.stringify({
          dni: datosFormu.dni,
        }),
  ...
  useEffect(() => {
    const loadPolicy = async () => {
      const response = await fetch(
        "http://149.50.132.147:3000/api/polizas/moto/porCliente",
        {
          method: "POST",
          body: JSON.stringify({
            id_client: clientId,
              ...
              {policysClient.map((policy, index) => {
                return (
                  <RowTable
                    ...
                    marca_moto={policy.motos.marca}
                    modelo_moto={policy.motos.modelo}
                    patente_moto={policy.motos.patente}
                    key={policy + index}
                  />
                );
              })}
